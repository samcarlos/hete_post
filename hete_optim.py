...
def gridsearch_hete_optim_action(X_train, tmt_control,  y_train):
    ...
    param_grid = dict(num_nodes = [32, 128, 256], dropout = [.5, .9], activation = [lelu, 'relu','tanh'], num_layers = [3,2])
    ...
    for params in grid:
        ...
        for train_index, test_index in kf.split(y_train):
            ...
            mod.fit([X_train[train_index], X_train[train_index][:,0]],
                    dummy_y_train[train_index], epochs = 50, verbose = 0)
    ...
    mod.fit([X_train, X_train[:,0]],
                    dummy_y_train, epochs = 50, verbose = 0)
